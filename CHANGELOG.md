...
- `MarkdownSourceMapSegment` 与 `SourceMapSegmentKind` 改为 source-map 内部实现细节，不再从 `@lint-md/parser` 导出；公开 API 仅保留可由 `MarkdownSourceMap` 实际使用的类型，且移除从未产生的 `generated` segment kind（#60）

### Tests

- `scripts/bench-source-map.mjs` 新增 URL 字段映射构建 benchmark：报告 16/32/64/128/256/512 KiB 的连续非实体 `&` destination，并在 Node 22 CI smoke 中以 128→512 KiB 中位耗时增长比率阻止平方级回归（#74）。
