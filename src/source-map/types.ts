...
export interface MarkdownSourceMap {
   ...
   * - {@link SourceMapConsistencyError} — the mapped text node's `value` has
   *   been modified since parsing; the map only covers the original value.
   *   (Only mapped text nodes are subject to this check; `position` mutation
   ...
   *   supported `text` node. No mapping is fabricated.
   * - {@link SourceMapConsistencyError} — the node has been modified since
