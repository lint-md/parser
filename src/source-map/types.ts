...
export interface MarkdownSourceMap {
   ...
   * `parseMdWithSourceMap()` call and that has a real source span.
   *
   * Failure modes:
   *
   * - {@link SourceMapUnavailableError} — the node belongs to another
   *   document, or has no source position (e.g. it was generated by a
   *   plugin or added after parsing). No mapping is fabricated.
   * - {@link SourceMapConsistencyError} — the mapped text node has been
   *   modified since parsing; the map only covers the original value.
   *
