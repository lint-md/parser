 ...
 * compressed segments that reconstruct their `value` from the raw Markdown
 * source.
 *
 * @public
 */
export interface MarkdownSourceMap {
   ...
   * - {@link SourceMapConsistencyError} — the mapped node's `value` has
   *   been modified since parsing; the map only covers the original value.
   *   (Only mapped value nodes are subject to this check; `position` mutation
